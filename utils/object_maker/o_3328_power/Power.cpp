/*
 * Power
 * Generated on: 2025-11-12 12:56:21
...
Power::Power(lwm2m_context_t &context, const OBJ_LINK_T &id): Instance(context, id) {
...
Object & Power::object(WppClient &ctx) {
	return ctx.registry().power();
}

Power * Power::instance(WppClient &ctx, ID_T instId) {
	Instance *inst = ctx.registry().power().instance(instId);
	if (!inst) return NULL;
	return static_cast<Power*>(inst);
}

Power * Power::createInst(WppClient &ctx, ID_T instId) {
	Instance *inst = ctx.registry().power().createInstance(instId);
	if (!inst) return NULL;
	return static_cast<Power*>(inst);
}

bool Power::removeInst(WppClient &ctx, ID_T instId) {
	return ctx.registry().power().remove(instId);
}

void Power::serverOperationNotifier(Instance *securityInst, ItemOp::TYPE type, const ResLink &resLink) {
...
void Power::userOperationNotifier(ItemOp::TYPE type, const ResLink &resLink) {
