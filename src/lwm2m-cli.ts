...
import { APIResponse, ClientInfo } from './types/lwm2m';
...
class LwM2MClientCLI {
  ...
  private async makeRequest<T = any>(endpoint: string, method: 'GET' | 'POST' | 'PUT' | 'DELETE' = 'GET', data?: any): Promise<APIResponse<T>> {
    try {
      const config = {
        method,
   ...
   */
  async deregisterClient(): Promise<void> {
  ...
  async getStatus(): Promise<void> {
    const response = await this.makeRequest<ClientInfo>('/api/client/status');
   ...
   */
  async writeValue(objectId: string, instanceId: string, resourceId: string, value: string): Promise<void> {
   ...
   */
  async readValue(objectId: string, instanceId: string, resourceId: string): Promise<void> {
  ...
  async listObjects(): Promise<void> {
    ...
    if (response.success && response.data) {
      ...
      console.log('\\n=== LwM2M Objects ===');
      Object.entries(config.objects).forEach(([objectId, objConfig]: [string, any]) => {
        console.log(`\\n📦 Object ${objectId}: ${objConfig.description}`);
        
        Object.entries(objConfig.instances).forEach(([instanceId, instConfig]: [string, any]) => {
          console.log(`  └─ Instance ${instanceId}: ${instConfig.description}`);
          
          Object.entries(instConfig.resources).forEach(([resourceId, resConfig]: [string, any]) => {
   ...
   */
  async getSignalKStatus(): Promise<void> {
