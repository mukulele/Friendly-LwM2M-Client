/*
 * Current
 * Generated on: 2025-11-12 12:46:16
...
Current::Current(lwm2m_context_t &context, const OBJ_LINK_T &id): Instance(context, id) {
...
Object & Current::object(WppClient &ctx) {
	return ctx.registry().current();
}

Current * Current::instance(WppClient &ctx, ID_T instId) {
	Instance *inst = ctx.registry().current().instance(instId);
	if (!inst) return NULL;
	return static_cast<Current*>(inst);
}

Current * Current::createInst(WppClient &ctx, ID_T instId) {
	Instance *inst = ctx.registry().current().createInstance(instId);
	if (!inst) return NULL;
	return static_cast<Current*>(inst);
}

bool Current::removeInst(WppClient &ctx, ID_T instId) {
	return ctx.registry().current().remove(instId);
}

void Current::serverOperationNotifier(Instance *securityInst, ItemOp::TYPE type, const ResLink &resLink) {
...
void Current::userOperationNotifier(ItemOp::TYPE type, const ResLink &resLink) {
