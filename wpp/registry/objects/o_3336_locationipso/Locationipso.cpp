...
#include "o_3336_locationipso/Locationipso.h"
...
#define TAG "Locationipso"

namespace wpp {

Locationipso::Locationipso(lwm2m_context_t &context, const OBJ_LINK_T &id): Instance(context, id) {
...
Object & Locationipso::object(WppClient &ctx) {
	return ctx.registry().locationipso();
}

Locationipso * Locationipso::instance(WppClient &ctx, ID_T instId) {
	Instance *inst = ctx.registry().locationipso().instance(instId);
	if (!inst) return NULL;
	return static_cast<Locationipso*>(inst);
}

Locationipso * Locationipso::createInst(WppClient &ctx, ID_T instId) {
	Instance *inst = ctx.registry().locationipso().createInstance(instId);
	if (!inst) return NULL;
	return static_cast<Locationipso*>(inst);
}

bool Locationipso::removeInst(WppClient &ctx, ID_T instId) {
	return ctx.registry().locationipso().remove(instId);
}

void Locationipso::serverOperationNotifier(Instance *securityInst, ItemOp::TYPE type, const ResLink &resLink) {
...
void Locationipso::userOperationNotifier(ItemOp::TYPE type, const ResLink &resLink) {
