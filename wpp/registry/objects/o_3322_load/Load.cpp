/*
 * Load
 * Generated on: 2025-11-12 12:46:17
...
Load::Load(lwm2m_context_t &context, const OBJ_LINK_T &id): Instance(context, id) {
...
Object & Load::object(WppClient &ctx) {
	return ctx.registry().load();
}

Load * Load::instance(WppClient &ctx, ID_T instId) {
	Instance *inst = ctx.registry().load().instance(instId);
	if (!inst) return NULL;
	return static_cast<Load*>(inst);
}

Load * Load::createInst(WppClient &ctx, ID_T instId) {
	Instance *inst = ctx.registry().load().createInstance(instId);
	if (!inst) return NULL;
	return static_cast<Load*>(inst);
}

bool Load::removeInst(WppClient &ctx, ID_T instId) {
	return ctx.registry().load().remove(instId);
}

void Load::serverOperationNotifier(Instance *securityInst, ItemOp::TYPE type, const ResLink &resLink) {
...
void Load::userOperationNotifier(ItemOp::TYPE type, const ResLink &resLink) {
